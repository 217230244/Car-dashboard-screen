...
u16 TP_Read_AD(u8 CMD)	  
{
	...
	Num = rx_data[0]<<8|rx_data[1];
//  Num=SPI1_ReadWriteByte(0xff)<<8;
//	Num|=SPI1_ReadWriteByte(0xff);
	Num>>=4;   	//只有高12位有效.
	CS2_Set();		//释放片选	 
	return(Num);   
}
...
u8 TP_Read_XY2(u16 *x,u16 *y) 
{
	u16 x1,y1;
 	u16 x2,y2;
 	u8 flag;
	//SPI1_SetSpeed(SPI_BaudRatePrescaler_32);//SPI 32分频
	hspi1.Instance->CR1&=0XFFC7;
	hspi1.Instance->CR1|=4<<3;	//设置SPI1速度 
	flag=TP_Read_XY(&x1,&y1);   
	if(flag==0)return(0);
	flag=TP_Read_XY(&x2,&y2);	   
	if(flag==0)return(0);
	//SPI1_SetSpeed(SPI_BaudRatePrescaler_4);//SP1  4分频
	hspi1.Instance->CR1&=0XFFC7;
	hspi1.Instance->CR1|=1<<3;	//设置SPI1速度 
...
u8 TP_Init(void)
{
	...
	if(Adujust)
	{
		...
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.170940;
			tp_dev.xoff=-10;
			tp_dev.yfac=0.252149;
			tp_dev.yoff=-10;
		}else if(USE_HORIZONTAL==1)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.170213;
			tp_dev.xoff=332;
			tp_dev.yfac=-0.253749;
			tp_dev.yoff=490;
		}else if(USE_HORIZONTAL==2)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.253602;
			tp_dev.xoff=-13;
			tp_dev.yfac=-0.169903;
			tp_dev.yoff=331;
		}else
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.250427;
			tp_dev.xoff=488;
			tp_dev.yfac=0.170213;
			tp_dev.yoff=-16;
		}
		return 0;
	}else
