...
static int isotp_receive_single_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    isotp_user_debug("single frame len:%d\r\n", link->receive_size)    ;
    return ISOTP_RET_OK;
}
...
static int isotp_receive_consecutive_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    if((link->receive_offset % 100) == 0)
    {
        printf(">%d\r\n", link->receive_offset);        //TODO: 调试用
    }
...
// 阻塞发送数据
int isotp_blocking_send(IsoTpLink *link, const uint8_t payload[], uint16_t size, uint32_t timeout_ms)
{
    int ret;
    uint32_t start_time = isotp_user_get_ms();

    // 初始化发送
    ret = isotp_send_with_id(link, link->send_arbitration_id, payload, size);
    if (ret != ISOTP_RET_OK)
    {
    ...
    {
        // 检查超时
        if (IsoTpTimeAfter(isotp_user_get_ms(), start_time + timeout_ms))
        {
            link->send_status = ISOTP_SEND_STATUS_ERROR; // 设置发送状态为错误
            printf("block timeout\r\n");
            return ISOTP_RET_TIMEOUT; // 返回超时错误
        }
        // printf("send status:%d\r\n", link->send_status);
        // isotp_poll(link); // 继续轮询
        iso_tp_server();
    }

    return ISOTP_RET_OK; // 发送成功
}
